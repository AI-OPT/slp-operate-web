...
public class StorageController {
    ...
    public String storageEdit(@PathVariable("id") String standedProdId, Model uiModel) {
        ...
        for (StorageGroupRes storageGroupRes : storageGroupResList) {
            ...
            paramSingleCond = new SysParamSingleCond(SysCommonConstants.COMMON_TENANT_ID,
                    ComCacheConstants.StateStorage.STORAGEGROUP_TYPR_CODE,
                    ComCacheConstants.StateStorage.PARAM_CODE, state);
            ...
            for (Short key : storageGroupRes.getStorageList().keySet()) {
                for (StorageRes storageRes : storageGroupRes.getStorageList().get(key)) {
                    ...
                    paramSingleCond = new SysParamSingleCond(SysCommonConstants.COMMON_TENANT_ID,
                            ComCacheConstants.StateStorage.STORAGE_TYPR_CODE,
                            ComCacheConstants.StateStorage.PARAM_CODE, storState);
    ...
    @ResponseBody
    public ResponseData<StorageGroupRes> addStorGroup(String standedProdId, String storageGroupName, HttpSession session) {
        IStorageSV storageSV = DubboConsumerFactory.getService(IStorageSV.class);
        STOStorageGroup storageGroup = new STOStorageGroup();
        storageGroup.setCreateId(AdminUtil.getAdminId(session));
        storageGroup.setStandedProdId(standedProdId);
        storageGroup.setStorageGroupName(storageGroupName);
        ...
        SysParamSingleCond paramSingleCond = new SysParamSingleCond(SysCommonConstants.COMMON_TENANT_ID,
                ComCacheConstants.StateStorage.STORAGEGROUP_TYPR_CODE,
                ComCacheConstants.StateStorage.PARAM_CODE, storGroupState);
        ...
        return new ResponseData<StorageGroupRes>(ResponseData.AJAX_STATUS_SUCCESS, "添加成功", storageGroupRes);
    }
    ...
    public ResponseData<StorageInfo> addStorage(HttpServletRequest request, HttpSession session) {
        ...
        BaseResponse baseResponse = storageSV.saveStorage(storage);//添加库存
        ResponseHeader header = baseResponse.getResponseHeader();
